...
def fetch_gst_details(gst_number):
    ...
    try:
        # Step 1: Open search page (best-effort)
        resp = session.get(search_page_url, headers=headers, timeout=10)
        if resp.status_code != 200:
            logging.warning("Search page returned status %s", resp.status_code)
        ...
        if response.status_code == 200:
            try:
                ...
                logging.exception("Failed to decode JSON from API at %s", api_url)
                return None

        logging.warning("API returned status %s for %s", response.status_code, api_url)
        return None

    except requests.RequestException:
        logging.exception("Network error when fetching GST details for %s", gst_number)
        return None
    except Exception:
        logging.exception("Unexpected error when fetching GST details for %s", gst_number)
        return None
